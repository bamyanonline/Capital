...
(function () {
  ...
  function referralId(users) {
    ...
    let value;
    do {
      value = "CAP" + Array.from({ length: 5 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("");
    } while (users.some(u => u.referralId === value || u.id === value));
    return value;
  }
  ...
  const backend = {
    ...
    async requestPasswordReset(email){return this.request("/api/auth/request-reset",{method:"POST",body:JSON.stringify({email})})},
  ...
  window.CapitalAPI = {
    ...
    async changePassword(a,b){return this.adapter.changePassword(a,b);},
    async requestPasswordReset(email){if(CAPITAL_CONFIG.mode!=="backend")throw new Error("BACKEND_REQUIRED");return backend.requestPasswordReset(email)},
    async resetPassword(token,p){if(CAPITAL_CONFIG.mode!=="backend")throw new Error("BACKEND_REQUIRED");return backend.resetPassword(token,p);}, async teamStats(){return this.adapter.teamStats();}, async listNotifications(){return this.adapter.listNotifications();}, async markNotificationRead(id){return this.adapter.markNotificationRead(id);},
